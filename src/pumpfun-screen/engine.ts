...
export function buildTimeline(data: ChartData, cfg: EngineConfig): FrameView[] {
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    const cam = camera[f];
    // Vibe skin: ~70 fat, clearly-readable candles (a "15m" perp-chart look)
    // rather than the pump.fun skin's dense 240-candle history. The recent
    // window is what shows; long pre-pump history is cropped to keep candles fat.
    const BASE_VIEW = 72;
    const viewCount = Math.max(48, Math.min(120, Math.round(BASE_VIEW * cam.zoom)));
    const godX = Math.max(
      2,
